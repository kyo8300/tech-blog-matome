...
Uber Engineering と LinkedIn Engineering は2026年9月時点でRSSフィードが提供されていません（フィードURL・代替URLとも404）。この2ソースは、フィード取得に失敗すると自動的に**一覧ページのHTMLから記事リンクを抽出するフォールバック**に切り替わります。一覧ページの記事一覧（見出しや十分な長さのリンクテキストを持つもの）を上から順に拾い、通常のフィード記事と同じように新着として取り込みます。日付が取得できない記事は取得時刻を公開日時として扱います。
