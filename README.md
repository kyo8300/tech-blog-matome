...
**一覧経路の新着判定について**: 一覧ページには過去記事もずっと並び続けるため、「DBに未登録＝新着」という通常のフィード経路の判定はそのままでは使えません（過去記事を毎回新着として大量に取り込み、要約APIのクレジットを浪費してしまいます）。そこでこの経路では、各ソースが「これまでに見た記事ID」の一覧（`listingSeenIds`。最大1000件、古いものから破棄）を保持し、**そこに無いIDだけを新着**として扱います。初回はさらに安全側に倒し、一覧の先頭（最新）1件だけを取り込みます。設定ページの各ソース行にある「**このソースの記事を削除して再取得**」ボタンを押すと、そのソースの記事・要約・チャット履歴を削除し、`listingSeenIds` を含めて未取得状態に戻せます（確認ダイアログあり）。
