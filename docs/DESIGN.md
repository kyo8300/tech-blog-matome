...
- `extractListingItems` の規則:
  ...
  4. 日付: アンカー自身、または最も近い祖先 `article` / `li` / `div`（3 階層まで）の中の `time[datetime]` を `Date.parse`。無ければ **テキスト日付**を探す: アンカーから祖先を 6 階層まで順に上り、各祖先の textContent に日付テキスト（`Sep 2, 2026` / `September 2, 2026` / `2 Sep 2026` / `2026-09-02` / `2026/09/02` の各形式。`src/lib/dateText.ts` の `findDateTexts(text): number[]`。ISO はタイムゾーンオフセット `Z` / `+09:00` / `-0700` まで一致文字列全体を `Date.parse` し、オフセット無しは UTC 扱い。年は 1990〜今年+1 の範囲外を日付とみなさない）が **ちょうど 1 件** 含まれる最初の祖先の値を採用する（複数含む祖先は一覧全体なので採用しない。LinkedIn の一覧はこの形）。それでも無ければ `undefined`。祖先ごとの `findDateTexts` の結果は `WeakMap<Element, number[]>` でメモ化し（同じ祖先を複数のアンカーが共有するため）、祖先の textContent が 20,000 字を超える要素は「一覧全体」とみなして評価せず打ち切る。
...
- `scripts/check-feeds.ts`: `listingUrl` を持つソースは主URL失敗時に一覧ページも取得し、jsdom で `extractListingItems` を実行して `形式=listing / 件数 / 最新タイトル` の行を追加する。件数欄は `12件（日付あり 12）` のように一覧から公開日が取れた件数も出す。さらに先頭1件の記事ページを元 URL で取得し、`extractPublishedAt`（§10）で公開日が取れるかを「記事ページの日付」列に `2026-09-10` または `取れず` として出す。
  - `--debug-dates`: `listingUrl` を持つソースについて、一覧ページと先頭1件の記事ページ（元 URL）に対し、(1) `<time>` 全文 (2) name/property/itemprop に date/time/publish/modified を含む `<meta>` (3) ld+json の @type と datePublished/dateModified/uploadDate (4) `__NEXT_DATA__` 等の JSON 内の日付らしきキー (5) 本文テキスト中の日付らしき文字列（前後30字）と `findDateTexts` の結果 (6) 一覧の先頭3件のアンカー outerHTML と最も近い祖先の textContent、を各最大10件で表示する。通常フローと exit code は変えない。`--fixture` を併用すると fetch せず `tests/fixtures/listing-uber-like.html` と `tests/fixtures/article.html` に対して自己テストする。一覧が 1 件以上取れれば、そのソースは失敗に数えない。
...
```
offscreen 側: `new DOMParser().parseFromString(html, "text/html")` → `<base href={url}>` を挿入 → `new Readability(doc).parse()` → `{ title, text: textContent を空白正規化, excerpt, publishedAt? }` を返す。`publishedAt` は Readability の前に `meta[property="article:published_time"]` → `meta[name="date"|"pubdate"|"publish-date"|"dc.date"]` → ld+json の `datePublished` → `article time[datetime]` / `time[datetime]` → **テキスト日付**（最初の `h1` から祖先を 5 階層まで上り、`findDateTexts` で日付テキストがちょうど 1 件含まれる最初の祖先の値。h1 が無い、または祖先で見つからなければ本文先頭 3000 字の最初の日付テキスト）の順で各候補を **1 つずつ評価し、`Date.parse` に成功した最初の値**（epoch ms）を返す（存在するが parse できない候補は飛ばして次へ）。共通実装は `src/lib/pageDate.ts` の `extractPublishedAt(doc)`。fetch は SW 側で行い、offscreen は解析だけ。HTML は 3MB で打ち切ってから送る。offscreen の利用は `acquire()` / `release()` の参照カウントで管理し、利用者がゼロになったときだけ `chrome.offscreen.closeDocument()`（例外は無視）。
...
- `tests/pageDate.test.ts`（`// @vitest-environment jsdom`）: meta → ld+json → time → h1 近傍のテキスト日付の優先順位 / LinkedIn 型（h1 の下の byline に `Sep 2, 2026`、フッターの関連記事に別の日付）で h1 近傍の日付が選ばれる
