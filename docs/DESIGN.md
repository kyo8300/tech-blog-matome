...
│   │                listingExtract.ts pageDate.ts dateText.ts domText.ts
...
- `src/lib/` は純粋TS。Node（scripts / vitest）でも動く。例外として `listingExtract.ts` / `pageDate.ts`（引数の `Document`）と `domText.ts`（引数の `Element`）は受け取った DOM ノードを操作する（グローバルの DOM には触れない。offscreen では DOMParser、scripts / tests では jsdom が渡す）。
...
- `extractListingItems` の規則:
  ...
  4. 日付: アンカー自身、または最も近い祖先 `article` / `li` / `div`（3 階層まで）の中の `time[datetime]` を `Date.parse`。無ければ **テキスト日付**を探す: アンカーから祖先を 6 階層まで順に上り、各祖先のテキスト（`script` / `style` / `noscript` / `template` の子孫を除き、要素境界に空白を挟んで連結。§10 と同じ `cleanTextContent`）に日付テキスト（`Sep 2, 2026` / `September 2, 2026` / `2 Sep 2026` / `2026-09-02` / `2026/09/02` の各形式。`src/lib/dateText.ts` の `findDateTexts(text): number[]`。ISO はタイムゾーンオフセット `Z` / `+09:00` / `-0700` まで一致文字列全体を `Date.parse` し、オフセット無しは UTC 扱い。年は 1990〜今年+1 の範囲外を日付とみなさない）が **ちょうど 1 件** 含まれる最初の祖先の値を採用する（複数含む祖先は一覧全体なので採用しない。LinkedIn の一覧はこの形）。それでも無ければ `undefined`。祖先ごとの `findDateTexts` の結果は `WeakMap<Element, number[]>` でメモ化し（同じ祖先を複数のアンカーが共有するため）、掃除済みテキスト（`cleanTextContent` の結果。祖先ごとにメモ化）が 20,000 字を超える要素は「一覧全体」とみなして評価せず打ち切る。
  4'. URL: 返す `url` は **絶対化した元の URL（フラグメントのみ除去。末尾スラッシュやクエリはそのまま）**。`normalizeUrl` は規則 1・1' のパターン判定と規則 5 の重複除去のキーにだけ使う（Uber は末尾スラッシュ無しの URL に 404 を返すため、正規化 URL で取得してはいけない）。`Article.url` にはこの元 URL を入れ、`Article.id` は正規化 URL の sha256。
...
- `scripts/check-feeds.ts`: `listingUrl` を持つソースは主URL失敗時に一覧ページも取得し、jsdom で `extractListingItems` を実行して `形式=listing / 件数 / 最新タイトル` の行を追加する。件数欄は `12件（日付あり 12）` のように一覧から公開日が取れた件数も出す。さらに先頭1件の記事ページを元 URL で取得し、`extractPublishedAt`（§10）で公開日が取れるかを「記事ページの日付」列に `2026-09-10` または `取れず` として出す。
  - `--debug-dates`: `listingUrl` を持つソースについて、一覧ページと先頭1件の記事ページ（元 URL）に対し、(1) `<time>` 全文 (2) name/property/itemprop に date/time/publish/modified を含む `<meta>` (3) ld+json の @type と datePublished/dateModified/uploadDate (4) `__NEXT_DATA__` 等の JSON 内の日付らしきキー（`\u0022` / `\"` を `"` に戻してから探索し、数値 epoch も対象） (5) 本文テキスト中の日付らしき文字列（前後30字）と `findDateTexts` の結果 (6) 一覧の先頭3件のアンカー outerHTML と最も近い祖先の textContent、を各最大10件で表示する。通常フローと exit code は変えない。`--fixture` を併用すると fetch せず `tests/fixtures/listing-uber-like.html` と `tests/fixtures/article.html` に対して自己テストする。一覧が 1 件以上取れれば、そのソースは失敗に数えない。
...
```
offscreen 側: `new DOMParser().parseFromString(html, "text/html")` → `<base href={url}>` を挿入 → `new Readability(doc).parse()` → `{ title, text: textContent を空白正規化, excerpt, publishedAt? }` を返す。`publishedAt` は Readability の前に `meta[property="article:published_time"]` → `meta[name="date"|"pubdate"|"publish-date"|"dc.date"]` → ld+json の `datePublished` → **埋め込み JSON の公開日キー**（ld+json 以外の `<script>` の中身を対象に、`\u0022` と `\"` を `"` に戻してから `"(publishedAt|datePublished|published_at|date_published|publishDate|publish_date|published_time|publishedDate)"\s*:\s*("[^"]{8,40}"|\d{10,13})` を探し、文字列は `Date.parse`、10桁は秒・13桁はミリ秒の epoch として最初に妥当なもの。`page_generated_at` や `createdAt` / `updatedAt` は対象外。Uber の記事ページはこの形） → `article time[datetime]` / `time[datetime]` → **テキスト日付**（最初の `h1` から祖先を 5 階層まで上り、`findDateTexts` で日付テキストがちょうど 1 件含まれる最初の祖先の値。対象テキストは `script` / `style` / `noscript` / `template` の子孫を除いて集め、要素境界には空白を 1 つ挟んで連結する（minify された SSR 出力で `TitleSep 2, 2026` のように連結されて取りこぼさないため）。h1 が無い、または祖先で見つからなければ本文先頭 3000 字の最初の日付テキスト）の順で各候補を **1 つずつ評価し、`Date.parse` に成功した最初の値**（epoch ms）を返す（存在するが parse できない候補は飛ばして次へ）。共通実装は `src/lib/pageDate.ts` の `extractPublishedAt(doc)`。fetch は SW 側で行い、offscreen は解析だけ。HTML は 3MB で打ち切ってから送る。offscreen の利用は `acquire()` / `release()` の参照カウントで管理し、利用者がゼロになったときだけ `chrome.offscreen.closeDocument()`（例外は無視）。
...
- `scripts/check-feeds.ts`（`npx tsx`）: `DEFAULT_SOURCES` を読み、各 feedUrl（`--all-alternates` で alt も）を 20s タイムアウト・ブラウザ風 UA で fetch → 表を出力: name / URL / HTTP status / content-type / 形式（rss2・atom・rdf・not-xml）/ 件数 / 最新タイトル+日付 / 本文全文あり(yes/no) / 記事ページの日付（`listingUrl` を持つソースのみ。`YYYY-MM-DD` または `取れず`）。主URLがひとつでも失敗したら exit 1（§9.5 の一覧フォールバックで 1 件以上取れたソースは失敗に数えない）。`listingUrl` を持つソースの一覧行・`--debug-dates` / `--fixture` の診断出力は §9.5 の check-feeds の項に定める。
...
- `tests/dateText.test.ts`: `findDateTexts` が `Sep 2, 2026` / `September 2, 2026` / `2 Sep 2026` / `2026-09-02` / `2026/09/02` を epoch ms に変換し、`2026-09-02T10:00:00Z` のような ISO も 1 件として数え、`Aug 13, 2024` と `Sep 2, 2026` が両方あれば 2 件返す / 数字だけの `12/31` や電話番号は拾わない
- `tests/pageDate.test.ts`（`// @vitest-environment jsdom`）: meta → ld+json → 埋め込み JSON → time → h1 近傍のテキスト日付の優先順位 / Uber 型（`<script>` 内に `\u0022publishedAt\u0022:\u00222026-09-11T16:47:43.218969Z\u0022` の **`\u0022` 形**と、`\"publishedAt\":\"…\"` の **`\"` 形**の両方をそれぞれテストし、`page_generated_at` `createdAt` `updatedAt` が同居）で publishedAt だけが選ばれる / 要素間に空白の無い `<h1>Title</h1><p>Sep 2, 2026</p>` でテキスト日付が取れる / 13桁 epoch の `"publishedAt":1757000000000` も取れる / LinkedIn 型（h1 の下の byline に `Sep 2, 2026`、フッターの関連記事に別の日付）で h1 近傍の日付が選ばれる
- `tests/listingExtract.test.ts`（`// @vitest-environment jsdom`）: LinkedIn 型の一覧（カード内に `<span>Sep 2, 2026</span>` のテキスト日付、time 無し）で publishedAt が取れる / 返る `url` が末尾スラッシュ付きの元 URL のまま（`https://www.uber.com/us/en/blog/signals-to-context/`）で、重複除去は正規化キーで行われる / fixture の一覧 HTML から、パターン一致かつ見出し/20文字以上のリンクだけが文書順・重複なしで取れる / 相対 href の絶対化 / `time[datetime]` の日付 / カテゴリリンク・ページネーションが落ちる / パターン不一致で 0 件 / nav・header・footer 内、hreflang 付き、listingUrl の祖先パス、`page=` 付きが落ちる / ld+json の ItemList・BlogPosting があればそれを優先し datePublished が publishedAt になる / `listingExcludePattern` に一致する URL が落ちる / Uber の実 URL 形（記事 `/us/en/blog/<slug>/`、カテゴリ `/us/en/blog/health/`、言語切替 `/es-ES/blog/engineering/`、トップ `/us/en/blog/`）で記事だけが残る / 列挙に無い 1 単語カテゴリ（`/us/en/blog/autonomous/`、見出し `Autonomous`）が規則 1'' で落ち、1 単語スラッグでも見出しが複数語の記事（`/us/en/blog/michelangelo/`、見出し `Michelangelo: Uber's ML Platform`）と ld+json 由来の 1 単語スラッグは残る
